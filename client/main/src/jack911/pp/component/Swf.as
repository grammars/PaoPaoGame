package jack911.pp.component
{
	...
	public class Swf
	{
		...
		protected var jl:JLoader = new JLoader();
		...
		public function load(url:String, className:String, autoRemoveTime:Object=null):void
		{
			...
			else
			{
				this.url = url;
				this.className = className;
				jl.add(JLoadTask.TYPE_CLASS_DOMAIN, url, Cfg.SAME_DOMAIN, 0, __domainGot);
				jl.start();
			}
		}
		
		private function __domainGot(task:JLoadTask):void
		{
			
			this.domain = task.result.getDomain();
