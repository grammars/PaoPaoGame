...
public abstract class GoodsContainer extends StGoodsContainer
{
	...
	/** 获取某种物品的总数量 */
	public int getTotalNum(int cfgId)
	{
		int total = 0;
		for(GoodsInfo item : items)
		{
			if(item.cfgId == cfgId) { total += item.num; }
		}
		return total;
	}
	
	/** 寻找同种物品 */
	public List<GoodsInfo> getSameItems(int cfgId)
	{
	...
	{
		List<GoodsInfo> sames = getSameItems(item.cfgId);
	...
	{
		Centre.msg.goods.sendOperException_G2C(result.errCode, type, owner.getBundle().cccid);
		return result;
	}
